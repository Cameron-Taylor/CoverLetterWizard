...
const MAX_RESUME_STORAGE_BYTES = 2 * 1024 * 1024; // 2MB
...
export default function CoverLetterForm({ onSubmit, disabled }: Props) {
  ...
  // Persist tell us about yourself
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      if (tellUsAboutYourself) {
        sessionStorage.setItem(STORAGE_KEYS.tellUsAboutYourself, tellUsAboutYourself);
      } else {
        sessionStorage.removeItem(STORAGE_KEYS.tellUsAboutYourself);
      }
    } catch {
  ...
  const validateJobUrl = (url: string): string | null => {
    if (!url.trim()) return "Job URL is required";
    try {
      const parsed = new URL(url);
      const host = parsed.hostname.toLowerCase();
      // Reject common auth/login/internal URLs
      if (
        host.includes("login") ||
        host.includes("auth") ||
        host.includes("accounts.") ||
        host.includes("mycompany") ||
        host.includes("internal")
      ) {
        return "This URL looks like it may require login. Please use the public job posting URL.";
      }
      if (parsed.protocol !== "https:" && parsed.protocol !== "http:") {
        return "Please use a valid http or https URL.";
      }
      return null;
    } catch {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    } else if (!ACCEPTED_TYPES.includes(resume.type)) {
      newErrors.resume = "Resume must be PDF, DOCX, or JPEG";
    }
    ...
    if (!hasJobUrl && !hasJobDescription) {
      newErrors.jobUrl =
        "Provide a job posting URL or paste the job description below (at least 100 characters).";
    } else if (hasJobUrl) {
    ...
    if (tone === "conversational" && !tellUsAboutYourself.trim()) {
      newErrors.tellUsAboutYourself = "Tell us a bit about yourself for a conversational tone";
    }
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          try {
            ...
            if (b64) {
              sessionStorage.setItem(
                STORAGE_KEYS.resume,
                JSON.stringify({ b64, name: file.name, type: file.type })
              );
            }
          } catch {
      ...
      } else if (file && file.size > MAX_RESUME_STORAGE_BYTES) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      {/* Resume upload */}
      <div>
        <label className="block text-sm font-medium text-wizard-mint mb-2">
          Resume
        </label>
        <input
          type="file"
        ...
        <p className="mt-1 text-xs text-gray-400">
          PDF, DOCX, or JPEG. Saved for this session (cleared when you close the tab).
        </p>
        {resume && (
          <p className="mt-1 text-xs text-wizard-mint/80">
            {resumeFromStorage ? "Using resume from this session" : "Ready"}: {resume.name}
          </p>
        )}
        {errors.resume && (
      ...
      {/* Job posting: URL or manual paste */}
      <div className="space-y-3">
        <label className="block text-sm font-medium text-wizard-mint">
          Job posting
        </label>
        <div>
          <input
            ...
            placeholder="Job listing URL Here (optional if pasting below)"
        ...
        <div>
          <p className="text-xs text-gray-400 mb-1">
            Or paste the job description (for Ashby, LinkedIn, etc.)
          </p>
          <textarea
      ...
      {/* Tone */}
      <div>
        <label className="block text-sm font-medium text-wizard-mint mb-3">
          Tone
        </label>
          ...
          {[
            ...
            <label key={opt.value} className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-wizard-primary border-gray-500 focus:ring-wizard-primary"
              />
              <span>{opt.label}</span>
            </label>
          ))}
      ...
      {/* Tell us about yourself - conditional */}
      {tone === "conversational" && (
        <div>
          <label className="block text-sm font-medium text-wizard-mint mb-2">
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              if (errors.tellUsAboutYourself)
                setErrors((prev) => ({ ...prev, tellUsAboutYourself: "" }));
            }}
            placeholder="Share a bit of your personality, what motivates you, or anything you'd like to stand out..."
            rows={4}
          ...
          />
          {errors.tellUsAboutYourself && (
            <p className="mt-1 text-sm text-red-400">{errors.tellUsAboutYourself}</p>
          )}
        </div>
      )}
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            ...
            { value: "direct" as const, label: "Direct (use hiring manager name if known)" },
          ].map((opt) => (
            <label key={opt.value} className="flex items-center gap-3 cursor-pointer">
              <input
                type="radio"
                name="opening"
                value={opt.value}
                checked={opening === opt.value}
                onChange={() => setOpening(opt.value)}
                className="w-4 h-4 text-wizard-primary border-gray-500 focus:ring-wizard-primary"
              />
              <span>{opt.label}</span>
            </label>
          ...
          {[
            ...
            <label key={opt.value} className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                name="emphasis"
                value={opt.value}
                checked={emphasis === opt.value}
                onChange={() => setEmphasis(opt.value)}
                className="w-4 h-4 text-wizard-primary border-gray-500 focus:ring-wizard-primary"
              />
              <span>{opt.label}</span>
            </label>
      ...
      <div className="pt-2">
        <button
          type="submit"
          disabled={disabled}
          className="w-full py-3.5 px-6 rounded-form bg-wizard-mint text-wizard-ink font-semibold hover:bg-white focus:outline-none focus:ring-2 focus:ring-wizard-mint focus:ring-offset-2 focus:ring-offset-wizard-ink disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-lg"
        >
          Create my cover letter
        </button>
        <p className="mt-2 text-center text-xs text-gray-500">
          We’ll tailor it to this role—you can edit before sending.
        </p>
      </div>
    </form>
  );
